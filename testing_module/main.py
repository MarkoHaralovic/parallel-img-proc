...
sliding_window_tester.init([image1], time_synchronize, lambda img: sliding_window_detection(img, (64, 64), 32, dummy_classifier))

inpainting_tester = FunctionTester()
mask = np.zeros(image1.shape, dtype=bool)
mask[100:150, 100:150] = True
inpainting_tester.init([image1], time_synchronize, lambda img: perform_inpainting(img, mask))
